...
Для каждой функции из каждого контракта в граф добавляется соответствующая этой функции вершина. В конце все вершины складываются в общий граф. Рёбер на этом этапе нет, функции на отдельные иструкции не раскладываются. 
...
Для начала генерируется граф ***CFG*** для каждой функции. На этом этапе функция разбивается на блоки с инструкциями:

<img src="./graphsPictures/withdrawBalance_cfg.png" width="70%">

Далее ***CFG*** используется для генерации ***ICFG***. В данном примере разницы между ними нет:

<img src="./graphsPictures/withdrawBalance_icfg.png" width="50%">

Однако, если внутри одной функции есть вызов другой функции контракта (неважно, публичной или приватной), этот вызов разобъётся на отдельные блоки. Это можно видеть в данном примере:
...
Будут сгенерированы следующие графы:
- ***CFG***:
...
Для каждой функции создаётся свой ***SDG*** граф.
...
В начале этой функиции вызывается `self.propagate_state_vars_used()`, использующий алгоритм *bfs*. В этом *bfs* делаются вызовы
...
Далее для каждого блока (вершины) из ***ICFG*** упрощаюстя базовые блоки, которые не нужны для противоречивого состояния.
