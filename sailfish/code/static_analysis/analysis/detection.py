...
class Detection:
                ...
                for function_2 in composed_graphs[function_1].keys():
                    ...
                    for composed_graph in composed_graph_list:
                        ...
                        self.detect_dao_read_write_dependencies(function_1, src_graph, function_2, target_graph,
                                                            ...
                                                            str(count), composed_graph[4], composed_graph[5])
                        count = count + 1
    ...
    def output_paths(self, matched_function, start_node, primary_function, end_node, call_node, all_preds, function_2_sdg, function_3_sdg):
        primary_icfg_obj = self._icfg_objects[primary_function]
        primary_icfg_root_nodes = primary_icfg_obj._root_nodes

        matched_icfg_obj = self._icfg_objects[function_3_sdg._function]
        matched_icfg_obj.connect_icfg(self._icfg_objects[matched_function])
    ...
    def detect_dao_read_write_dependencies(self, primary_function, primary_sdg, matched_function, matched_sdg, composed_sdg, call_node, all_predecessors, count, function_2_sdg, function_3_sdg):
        variable_nodes = self.get_all_variable_nodes(composed_sdg)
        dependencies = {}
        edge_list = []
        ...
        if len(dependencies['dependencies']) != 0:
            ...
            if self._dump_graph == True:
                self.print_dependency(self._graph_dir, primary_function, matched_function, composed_sdg, edge_list, name, function_3_sdg)
    ...
    def print_dependency(self, graph_dir, function_1, function_2, graph, edge_list, name=None, function_3=None):
        ...
        if function_3 is None:
            dot_file_name = function_1.name + "_" + function_2.name + name + ".dot"
        else:
            dot_file_name = function_1.name + "_" + function_3._function.name + "_" + function_2.name + name + ".dot"
